...
raw/
  jurisdictions/
    ...
    us/        # U.S.-side documents (1040, W-2, 8879, IRS notices)
  asset_classes/
...
directories after a successful copy. The fact-extraction read path
keeps the legacy layout supported indefinitely; the migration is
about cleanliness and forward-compatibility with future jurisdictions
(UK, FR, ...), not a required upgrade step.

## Demo posture

This folder ships the checked-in synthetic config for the public
`demo-2025` workspace. The values are public-safe and exist so the
test suite can run end-to-end without a real private workspace.
